...
# Embedded HUD font — linked into libmop.a only when the bake tool
# has produced a generated source file (build/fonts/jbm_regular.mfa.c).
# The trigger is `make fonts` (which depends on the JBM TTF being
# present at assets/fonts/JetBrainsMono-Regular.ttf).  When absent,
# mop_font_hud() returns NULL and host code falls back to disk loads.
HUD_EMBED_SRC := $(BUILD_DIR)/fonts/jbm_regular.mfa.c
HUD_EMBED_OBJ := $(OBJ_DIR)/util/embedded_hud_font.o
ifneq ($(wildcard $(HUD_EMBED_SRC)),)
  CORE_OBJS += $(HUD_EMBED_OBJ)
  CFLAGS    += -DMOP_HAS_EMBEDDED_HUD_FONT=1
  # Force src/core/font.c to recompile when the embed appears or
  # changes — its mop_font_hud() switches between the embedded and
  # the no-op path based on the define above, so a stale .o would
  # silently keep returning NULL.
  $(OBJ_DIR)/core/font.o: $(HUD_EMBED_SRC)
endif

# -----------------------------------------------------------------------------
# Default target — static library only
# -----------------------------------------------------------------------------
.PHONY: all lib clean install test torture tools shaders conformance conformance-run docs-check docs-check-code ci-linux fonts
...
# Compile the embedded HUD font source generated by `make fonts`.
# Suppress the size warnings from the giant constant array literal —
# stb's STB_TRUETYPE_IMPLEMENTATION quirk also lives here.
$(HUD_EMBED_OBJ): $(HUD_EMBED_SRC) | obj_dirs
...
# Fonts
#
# Bakes every TTF listed below into a .mfa atlas under build/fonts/.
# The HUD font (JetBrains Mono Regular) is also emitted as a C
# header (build/fonts/hud_font.mfa.h) so it can be embedded into
# libmop via -DMOP_HAS_EMBEDDED_HUD_FONT and a thin shim TU.
#
# Drop the OFL-licensed JetBrainsMono-Regular.ttf at:
#   assets/fonts/JetBrainsMono-Regular.ttf
# (and Medium.ttf if you want the bold variant).
# -----------------------------------------------------------------------------
...
# JetBrains Mono Regular — HUD/navigator/labels.  --name hud_font
# triggers the embed-header emission so libmop can link it directly.
$(FONT_OUT_DIR)/jbm_regular.mfa: $(FONT_SRC_DIR)/JetBrainsMono-Regular.ttf $(FONT_BAKE_BIN) | $(FONT_OUT_DIR)
	$(FONT_BAKE_BIN) $< --out $@ --glyphs ascii+latin1 --size 64 --padding 4 --name hud_font
