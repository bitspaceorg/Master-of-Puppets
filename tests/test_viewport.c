...
static void test_create_destroy(void) {
    ...
    MopViewportDesc desc = { .width = 320, .height = 240, .backend = MOP_BACKEND_CPU };
...
static void test_create_zero_size(void) {
    ...
    MopViewportDesc desc = { .width = 0, .height = 240, .backend = MOP_BACKEND_CPU };
    MopViewport *vp = mop_viewport_create(&desc);
    TEST_ASSERT(vp == NULL);
    TEST_END();
}
...
static void test_resize(void) {
    ...
    MopViewportDesc desc = { .width = 64, .height = 64, .backend = MOP_BACKEND_CPU };
    MopViewport *vp = mop_viewport_create(&desc);
    TEST_ASSERT(vp != NULL);
...
static void test_add_remove_mesh(void) {
    ...
    TEST_ASSERT(vp != NULL);
    MopMeshDesc md = {
        .vertices = TRI_VERTS, .vertex_count = 3,
        .indices = TRI_IDX, .index_count = 3,
        .object_id = 42
    };
...
static void test_pick_cube(void) {
    ...
    mop_viewport_set_camera(vp,
        (MopVec3){0, 0, 3}, (MopVec3){0, 0, 0}, (MopVec3){0, 1, 0},
        60.0f, 0.1f, 100.0f);

    MopMeshDesc md = {
        .vertices = CUBE_VERTS, .vertex_count = 24,
        .indices = CUBE_IDX, .index_count = 36,
        .object_id = 1
    };
