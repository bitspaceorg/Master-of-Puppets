...
```
  ┌──────────────────────────────┐
  │        Application           │   examples/rotating_cube.c
  ├──────────────────────────────┤
  │      Viewport Public API     │   include/mop/*.h
  ├──────────────────────────────┤
  │       Viewport Core          │   src/viewport/
  ├──────────────────────────────┤
...
Defined in `src/viewport/`.  Owns the scene graph (flat mesh array), camera state, and rendering orchestration.  Calls the RHI to create buffers, begin frames, issue draw calls, and read back results.  Never calls backend APIs directly.
