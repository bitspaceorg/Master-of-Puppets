...
**Pointer-stable scene pool** — `vp->meshes` is an array of `MopMesh *`, each struct heap-allocated individually. Reallocating the pointer array on grow does **not** invalidate any `MopMesh *` the application holds. Freed slots are tracked in an O(1) free list and reused on next add. Every mesh carries its own `slot_index` and a back-pointer to its viewport so `mop_mesh_set_*` calls can auto-acquire the scene lock without the caller providing the viewport handle. Instanced meshes use the same design.
