...
- [x] **MOP_ENV_PROCEDURAL_SKY rendering on Vulkan — diagnostics + auto-recovery landed.** Reported by the Made-in-Heaven integration: `set_environment(type=PROCEDURAL_SKY) → set_procedural_sky(...) → set_environment_background(true)` left the clear-color background instead of the sky on Vulkan. Couldn't repro on CPU (covered by `tests/test_procedural_sky.c`). Hardening shipped: (1) `MOP_ERROR` when `texture_create_hdr` returns NULL during sky generation, naming the most likely cause (R32G32B32A32_SFLOAT not sampled-image-filterable, or staging upload failure); (2) one-shot `MOP_WARN` from `pass_background` when the GPU skybox path can't draw (`draw_skybox=NULL` or `env_texture=NULL`), with both pointer values logged; (3) defensive auto-recovery in `pass_background` — if `env_type=PROCEDURAL_SKY` + `env_hdr_data` set + `env_texture=NULL` on a non-CPU backend, regenerate the sky once before falling back. Next host hitting this gets a clear log line and a self-healing path; if the Vulkan upload genuinely can't succeed, the WARN names exactly which pointer is missing.
