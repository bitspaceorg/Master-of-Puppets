...
slug: "reference-interaction-input"
...
```
include/mop/interact/input.h      — Public types and API
...
| `MOP_INPUT_SCROLL`           | `scroll`             | Scroll wheel / pinch-to-zoom (positive = zoom in) |
| `MOP_INPUT_SCROLL_ORBIT`     | `dx`, `dy`           | Two-finger trackpad scroll → orbit camera         |
...
### Trackpad Gesture Mapping

On macOS, SDL3 reports trackpad gestures as mouse events:

- **Two-finger scroll** → `MOP_INPUT_SCROLL_ORBIT` with `dx`/`dy` — orbits the camera on both axes
- **Pinch-to-zoom** (Ctrl+scroll on macOS) → `MOP_INPUT_SCROLL` with `scroll` — zooms the camera
