...
| `picking.h`       | Object picking via ID buffer                  | [Picking](reference-interaction-picking)        |
| `light.h`         | Multi-light system (directional, point, spot) | [Light](reference-core-light)                   |
| `input.h`         | Input events, output events, interaction      | [Input](reference-interaction-input)            |
| `gizmo.h`         | TRS manipulation gizmo                        | [Gizmo](reference-interaction-gizmo)            |
| `camera.h`        | Orbit camera                                  | [Camera](reference-interaction-camera)          |
| `camera_query.h`  | Camera state snapshot, ray generation         | [Camera Query](reference-query-camera-query)    |
| `display.h`       | Display settings and visualization modes      | [Display](reference-core-display)               |
| `overlay.h`       | Built-in and custom overlays                  | [Overlay](reference-render-overlay)             |
| `material.h`      | PBR-inspired material descriptor              | [Material](reference-core-material)             |
| `vertex_format.h` | Flexible vertex attributes                    | [Vertex Format](reference-core-vertex-format)   |
| `loader.h`        | OBJ and binary mesh loading                   | [Loader](reference-io-loader)                   |
| `query.h`         | Read-only mesh introspection                  | [Query](reference-query-query)                  |
| `snapshot.h`      | Zero-copy scene iteration                     | [Snapshot](reference-query-snapshot)            |
| `spatial.h`       | AABB, frustum culling, raycasting             | [Spatial](reference-query-spatial)              |
| `undo.h`          | Transform undo/redo stack                     | [Undo](reference-interaction-undo)              |
| `pipeline.h`      | Custom render pass hooks                      | [Pipeline](reference-render-pipeline)           |
| `postprocess.h`   | Gamma, tonemapping, vignette, fog             | [Post-Processing](reference-render-postprocess) |
| `log.h`           | Logging levels and callbacks                  | [Log](reference-util-log)                       |
| `profile.h`       | Frame timing statistics                       | [Profile](reference-util-profile)               |
...
Feeds a platform input event into the viewport's interaction state machine. See [Input](reference-interaction-input) for the full event type list and state machine.
...
Dequeues the next output event. Returns `false` when the queue is empty. See [Input](reference-interaction-input) for the full output event type list.
