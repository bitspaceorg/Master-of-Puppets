...
static int gl_context_create(void) {
  ...
  EGLint ctx_attrs[] = {EGL_CONTEXT_MAJOR_VERSION,
                        3,
...
static uint8_t *render_scene(MopBackendType backend, int w, int h, int *out_w,
                             int *out_h) {
  ...
  mop_viewport_set_camera(vp, (MopVec3){2.0f, 2.0f, 3.0f},
                          (MopVec3){0.0f, 0.0f, 0.0f},
                          (MopVec3){0.0f, 1.0f, 0.0f}, 60.0f, 0.1f, 50.0f);

  mop_viewport_set_clear_color(vp, (MopColor){0.15f, 0.15f, 0.2f, 1.0f});

  MopMeshDesc mesh = {.vertices = CUBE_VERTICES,
...
static void compare(const char *name_a, const uint8_t *a, const char *name_b,
