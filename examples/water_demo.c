...
static void water_sim_create(WaterSim *ws, int resolution, float extent) {
    ...
    for (int z = 0; z < n - 1; z++) {
        for (int x = 0; x < n - 1; x++) {
            ...
            uint32_t br = bl + 1;
            ws->indices[ii++] = tl; ws->indices[ii++] = bl; ws->indices[ii++] = tr;
            ws->indices[ii++] = tr; ws->indices[ii++] = bl; ws->indices[ii++] = br;
        }
    }
    ...
    ws->wave_speed     = 1.5f;
    ws->opacity        = 0.65f;
    ws->color_r = 0.08f; ws->color_g = 0.25f; ws->color_b = 0.55f;
}
...
typedef struct { float x,y,z, vx,vy,vz, life,maxlife,size, r,g,b,a; bool alive; } Ptcl;
...
static void fire_create(FireSim *f, uint32_t max_p) {
    ...
    f->max_p = max_p; f->rate = 60; f->active = true;
    f->rng = 0xCAFE1234;
}

static void fire_free(FireSim *f) {
    free(f->pool); free(f->verts); free(f->indices);
}

static void fire_update(FireSim *f, float dt,
    ...
    for (uint32_t i = 0; i < f->max_p; i++) {
        ...
        p->r = 1.0f; p->g = 0.8f * (1.0f - t) + 0.1f * t;
        p->b = 0.2f * (1.0f - t);
        p->a = 1.0f - t;
    }
    if (f->active) {
        ...
        while (f->accum >= 1.0f) {
            f->accum -= 1.0f;
            for (uint32_t i = 0; i < f->max_p; i++) {
                if (f->pool[i].alive) continue;
                Ptcl *p = &f->pool[i];
                p->alive = true; p->x = f->px; p->y = f->py; p->z = f->pz;
                p->vx = frand_r(&f->rng, -0.3f, 0.3f);
                p->vy = frand_r(&f->rng,  1.5f, 3.0f);
                p->vz = frand_r(&f->rng, -0.3f, 0.3f);
                p->life = 0; p->maxlife = frand_r(&f->rng, 0.5f, 1.5f);
                p->size = 0.5f; p->r = 1; p->g = 0.8f; p->b = 0.2f; p->a = 1;
                break;
            }
        }
    }
    ...
    for (uint32_t i = 0; i < f->max_p; i++) {
        ...
        f->verts[vi+0] = (MopVertex){.position = {p->x-rx-ux, p->y-ry-uy, p->z-rz-uz},
                                      .normal = n, .color = c};
        f->verts[vi+1] = (MopVertex){.position = {p->x+rx-ux, p->y+ry-uy, p->z+rz-uz},
                                      .normal = n, .color = c};
        f->verts[vi+2] = (MopVertex){.position = {p->x+rx+ux, p->y+ry+uy, p->z+rz+uz},
                                      .normal = n, .color = c};
        f->verts[vi+3] = (MopVertex){.position = {p->x-rx+ux, p->y-ry+uy, p->z-rz+uz},
                                      .normal = n, .color = c};

        f->indices[ii] = vi; f->indices[ii+1] = vi+1; f->indices[ii+2] = vi+2;
        f->indices[ii+3] = vi+2; f->indices[ii+4] = vi+3; f->indices[ii+5] = vi;
        vi += 4; ii += 6;
    }
    f->vc = vi; f->ic = ii;
}
...
static const MopVertex ISLAND_VERTS[] = {
    {.position={-0.8f,0.6f, 0.8f},.normal={0,1,0},.color={0.22f,0.55f,0.15f,1}},
    {.position={ 0.8f,0.6f, 0.8f},.normal={0,1,0},.color={0.22f,0.55f,0.15f,1}},
    {.position={ 0.8f,0.6f,-0.8f},.normal={0,1,0},.color={0.22f,0.55f,0.15f,1}},
    {.position={-0.8f,0.6f,-0.8f},.normal={0,1,0},.color={0.22f,0.55f,0.15f,1}},
    {.position={-0.8f,-0.4f,0.8f},.normal={0,0,1},.color={0.45f,0.32f,0.18f,1}},
    {.position={ 0.8f,-0.4f,0.8f},.normal={0,0,1},.color={0.45f,0.32f,0.18f,1}},
    {.position={ 0.8f, 0.6f,0.8f},.normal={0,0,1},.color={0.45f,0.32f,0.18f,1}},
    {.position={-0.8f, 0.6f,0.8f},.normal={0,0,1},.color={0.45f,0.32f,0.18f,1}},
    {.position={ 0.8f,-0.4f,-0.8f},.normal={0,0,-1},.color={0.42f,0.30f,0.16f,1}},
    {.position={-0.8f,-0.4f,-0.8f},.normal={0,0,-1},.color={0.42f,0.30f,0.16f,1}},
    {.position={-0.8f, 0.6f,-0.8f},.normal={0,0,-1},.color={0.42f,0.30f,0.16f,1}},
    {.position={ 0.8f, 0.6f,-0.8f},.normal={0,0,-1},.color={0.42f,0.30f,0.16f,1}},
    {.position={0.8f,-0.4f, 0.8f},.normal={1,0,0},.color={0.44f,0.31f,0.17f,1}},
    {.position={0.8f,-0.4f,-0.8f},.normal={1,0,0},.color={0.44f,0.31f,0.17f,1}},
    {.position={0.8f, 0.6f,-0.8f},.normal={1,0,0},.color={0.44f,0.31f,0.17f,1}},
    {.position={0.8f, 0.6f, 0.8f},.normal={1,0,0},.color={0.44f,0.31f,0.17f,1}},
    {.position={-0.8f,-0.4f,-0.8f},.normal={-1,0,0},.color={0.44f,0.31f,0.17f,1}},
    {.position={-0.8f,-0.4f, 0.8f},.normal={-1,0,0},.color={0.44f,0.31f,0.17f,1}},
    {.position={-0.8f, 0.6f, 0.8f},.normal={-1,0,0},.color={0.44f,0.31f,0.17f,1}},
    {.position={-0.8f, 0.6f,-0.8f},.normal={-1,0,0},.color={0.44f,0.31f,0.17f,1}},
    {.position={-0.8f,-0.4f,-0.8f},.normal={0,-1,0},.color={0.35f,0.25f,0.12f,1}},
    {.position={ 0.8f,-0.4f,-0.8f},.normal={0,-1,0},.color={0.35f,0.25f,0.12f,1}},
    {.position={ 0.8f,-0.4f, 0.8f},.normal={0,-1,0},.color={0.35f,0.25f,0.12f,1}},
    {.position={-0.8f,-0.4f, 0.8f},.normal={0,-1,0},.color={0.35f,0.25f,0.12f,1}},
};
...
int main(int argc, char *argv[]) {
    ...
    SDL_Window *window = SDL_CreateWindow(
        "MOP — Water Simulation", 960, 720, SDL_WINDOW_RESIZABLE);
    ...
    if (backend_env) {
        if (strcmp(backend_env, "vulkan") == 0) backend = MOP_BACKEND_VULKAN;
        else if (strcmp(backend_env, "opengl") == 0) backend = MOP_BACKEND_OPENGL;
    }
    ...
    mop_viewport_set_camera(vp,
        (MopVec3){ 5, 4, 7 }, (MopVec3){ 0, 0.3f, 0 },
    ...
    mop_viewport_add_mesh(vp, &(MopMeshDesc){
        .vertices = ISLAND_VERTS, .vertex_count = 24,
        .indices = ISLAND_IDX, .index_count = 36, .object_id = 1 });
    ...
    mop_viewport_set_fog(vp, &(MopFogParams){
        .color = { 0.35f, 0.55f, 0.80f, 1 },
        .near_dist = 12.0f, .far_dist = 50.0f });
    ...
    fire.px = 0; fire.py = 0.6f; fire.pz = 0;

    FireSim smoke;
    fire_create(&smoke, 256);
    smoke.px = 0; smoke.py = 1.2f; smoke.pz = 0;
    smoke.rate = 30;
    ...
    MopMesh *marker_fire = mop_viewport_add_mesh(vp, &(MopMeshDesc){
        .vertices = marker_verts, .vertex_count = 6,
        .indices = marker_indices, .index_count = 24,
        .object_id = MARKER_BASE_ID });
    mop_mesh_set_position(marker_fire, (MopVec3){ fire.px, fire.py, fire.pz });

    MopMesh *marker_smoke = mop_viewport_add_mesh(vp, &(MopMeshDesc){
        .vertices = marker_verts, .vertex_count = 6,
        .indices = marker_indices, .index_count = 24,
    ...
    int btn_amp_up   = ui_toolbar_button(&toolbar, "Amplitude +", UI_BTN_MOMENTARY, 0, false);
    int btn_amp_dn   = ui_toolbar_button(&toolbar, "Amplitude -", UI_BTN_MOMENTARY, 0, false);
    int btn_freq_up  = ui_toolbar_button(&toolbar, "Frequency +", UI_BTN_MOMENTARY, 0, false);
    int btn_freq_dn  = ui_toolbar_button(&toolbar, "Frequency -", UI_BTN_MOMENTARY, 0, false);
    int btn_spd_up   = ui_toolbar_button(&toolbar, "Speed +",     UI_BTN_MOMENTARY, 0, false);
    int btn_spd_dn   = ui_toolbar_button(&toolbar, "Speed -",     UI_BTN_MOMENTARY, 0, false);
    int btn_opac_up  = ui_toolbar_button(&toolbar, "Opacity +",   UI_BTN_MOMENTARY, 0, false);
    int btn_opac_dn  = ui_toolbar_button(&toolbar, "Opacity -",   UI_BTN_MOMENTARY, 0, false);
    ...
    while (running) {
        ...
        while (SDL_PollEvent(&ev)) {
            ...
            if (ev.type == SDL_EVENT_KEY_DOWN) {
                ...
                case SDLK_D: ui_toolbar_toggle(&toolbar, btn_amp_up);  break;
                case SDLK_A: ui_toolbar_toggle(&toolbar, btn_amp_dn);  break;
                case SDLK_W: ui_toolbar_toggle(&toolbar, btn_freq_up); break;
                case SDLK_S: ui_toolbar_toggle(&toolbar, btn_freq_dn); break;
                case SDLK_E: ui_toolbar_toggle(&toolbar, btn_spd_up);  break;
                case SDLK_Q: ui_toolbar_toggle(&toolbar, btn_spd_dn);  break;
                case SDLK_P: ui_toolbar_toggle(&toolbar, btn_opac_up); break;
                case SDLK_O: ui_toolbar_toggle(&toolbar, btn_opac_dn); break;
        ...
        if (ui_toolbar_fired(&toolbar, btn_amp_up)) {
            ...
            if (water.wave_amplitude > 1.0f) water.wave_amplitude = 1.0f;
        }
        if (ui_toolbar_fired(&toolbar, btn_amp_dn)) {
            water.wave_amplitude -= 0.02f;
            if (water.wave_amplitude < 0.01f) water.wave_amplitude = 0.01f;
        }
        if (ui_toolbar_fired(&toolbar, btn_freq_up)) {
            water.wave_frequency += 0.2f;
            if (water.wave_frequency > 10.0f) water.wave_frequency = 10.0f;
        }
        if (ui_toolbar_fired(&toolbar, btn_freq_dn)) {
            water.wave_frequency -= 0.2f;
            if (water.wave_frequency < 0.2f) water.wave_frequency = 0.2f;
        }
        if (ui_toolbar_fired(&toolbar, btn_spd_up)) {
            water.wave_speed += 0.2f;
            if (water.wave_speed > 5.0f) water.wave_speed = 5.0f;
        }
        if (ui_toolbar_fired(&toolbar, btn_spd_dn)) {
            water.wave_speed -= 0.2f;
            if (water.wave_speed < 0.1f) water.wave_speed = 0.1f;
        }
        if (ui_toolbar_fired(&toolbar, btn_opac_up)) {
            water.opacity += 0.05f;
            if (water.opacity > 1.0f) water.opacity = 1.0f;
        }
        if (ui_toolbar_fired(&toolbar, btn_opac_dn)) {
            water.opacity -= 0.05f;
            if (water.opacity < 0.05f) water.opacity = 0.05f;
        }
        ...
        /* Water mesh */
        if (!water_mesh) {
            water_mesh = mop_viewport_add_mesh(vp, &(MopMeshDesc){
                .vertices     = water.verts,
            ...
            mop_mesh_update_geometry(water_mesh, vp,
                water.verts, water_vertex_count(&water),
                water.indices, water_index_count(&water));
        }
        if (water_mesh) mop_mesh_set_opacity(water_mesh, water.opacity);

        /* Fire mesh */
        if (fire.vc > 0) {
            if (!fire_mesh) {
                fire_mesh = mop_viewport_add_mesh(vp, &(MopMeshDesc){
                    .vertices = fire.verts, .vertex_count = fire.vc,
                    .indices = fire.indices, .index_count = fire.ic,
                    .object_id = 3 });
                if (fire_mesh) mop_mesh_set_blend_mode(fire_mesh, MOP_BLEND_ADDITIVE);
            } else {
                mop_mesh_update_geometry(fire_mesh, vp,
                    fire.verts, fire.vc, fire.indices, fire.ic);
            }
        } else if (fire_mesh) {
        ...
        if (smoke.vc > 0) {
            if (!smoke_mesh) {
                smoke_mesh = mop_viewport_add_mesh(vp, &(MopMeshDesc){
                    .vertices = smoke.verts, .vertex_count = smoke.vc,
                    .indices = smoke.indices, .index_count = smoke.ic,
                    .object_id = 4 });
                if (smoke_mesh) {
                ...
                mop_mesh_update_geometry(smoke_mesh, vp,
                    smoke.verts, smoke.vc, smoke.indices, smoke.ic);
            }
        } else if (smoke_mesh) {
    ...
    /* ---- Cleanup ---- */
    if (water_mesh) mop_viewport_remove_mesh(vp, water_mesh);
    if (fire_mesh) mop_viewport_remove_mesh(vp, fire_mesh);
    if (smoke_mesh) mop_viewport_remove_mesh(vp, smoke_mesh);
