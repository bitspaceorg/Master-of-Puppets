...
static void print_mat4(const char *label, MopMat4 m) {
  ...
  for (int row = 0; row < 4; row++) {
    printf("    [%8.4f %8.4f %8.4f %8.4f]\n", m.d[0 * 4 + row],
           m.d[1 * 4 + row], m.d[2 * 4 + row], m.d[3 * 4 + row]);
  }
}
...
static const char *blend_mode_str(MopBlendMode m) {
  switch (m) {
  case MOP_BLEND_OPAQUE:
    return "opaque";
  case MOP_BLEND_ALPHA:
    return "alpha";
  ...
  for (uint32_t mi = 0; mi < num_meshes; mi++) {
    ...
    if (verts) {
      ...
      for (uint32_t vi = 0; vi < nv; vi++) {
        ...
        MopVec4 wp =
            mop_mat4_mul_vec4(world, (MopVec4){lp.x, lp.y, lp.z, 1.0f});
        printf("  v %.6f %.6f %.6f\n", wp.x, wp.y, wp.z);
      }
    }
...
static void exporter_setup(MopViewport *vp, void *ctx) {
  ...
  /* Camera */
  mop_viewport_set_camera(vp, (MopVec3){3.0f, 3.0f, 5.0f},
  ...
  printf("[exporter] E=export to console  S=save export.mop  W=wireframe  "
         "Q/Esc=quit\n");
}
...
static void write_mat4(FILE *f, const char *indent, MopMat4 m) {
  ...
  for (int col = 0; col < 4; col++) {
    fprintf(f, "%s    %.6f %.6f %.6f %.6f\n", indent, m.d[col * 4 + 0],
            m.d[col * 4 + 1], m.d[col * 4 + 2], m.d[col * 4 + 3]);
  }
  fprintf(f, "%s]\n", indent);
}
    ...
    if (verts) {
      ...
      for (uint32_t vi = 0; vi < nv; vi++) {
        ...
        fprintf(f,
                ...
                v->normal.y, v->normal.z, v->color.r, v->color.g, v->color.b,
                v->color.a, v->u, v->v);
      }
      fprintf(f, "    }\n");
    }
