...
static const MopColor TARGET_COLORS[] = {
    {0.95f, 0.20f, 0.20f, 1}, {0.20f, 0.90f, 0.30f, 1},
    {0.25f, 0.40f, 0.95f, 1}, {0.95f, 0.85f, 0.10f, 1},
    {0.90f, 0.30f, 0.90f, 1}, {0.20f, 0.90f, 0.90f, 1},
    {1.00f, 0.55f, 0.10f, 1}, {0.50f, 0.90f, 0.20f, 1},
};
...
static void update_targets(MopViewport *vp, GameState *gs, float dt) {
  ...
  for (int i = 0; i < MAX_TARGETS; i++) {
    ...
    if (t->base_pos.x < -ARENA_HALF)
      t->base_pos.x = -ARENA_HALF;
    if (t->base_pos.x > ARENA_HALF)
      t->base_pos.x = ARENA_HALF;
    if (t->base_pos.z < -ARENA_HALF)
      t->base_pos.z = -ARENA_HALF;
    if (t->base_pos.z > ARENA_HALF)
      t->base_pos.z = ARENA_HALF;
...
static void hud_draw(SDL_Renderer *renderer, int win_w, int win_h,
  ...
  SDL_SetRenderDrawColor(renderer, gs->mode == MODE_PLAY ? 100 : 200,
                         gs->mode == MODE_PLAY ? 220 : 200,
                         gs->mode == MODE_PLAY ? 100 : 60, 255);
  ...
  if (gs->mode == MODE_PLAY && !gs->game_over) {
    const char *hint =
        "WASD=move  Mouse=look  Click=shoot  Space=jump  Tab=editor  R=restart";
    SDL_SetRenderDrawColor(renderer, 0, 0, 0, 120);
...
static void enter_play_mode(SDL_Window *window, MopViewport *vp,
                            GameState *gs) {
  gs->mode = MODE_PLAY;
...
static void enter_editor_mode(SDL_Window *window, MopViewport *vp,
                              GameState *gs) {
  gs->mode = MODE_EDITOR;
...
int main(int argc, char *argv[]) {
  ...
  SDL_Window *window = SDL_CreateWindow("MOP — Target Blaster", win_w, win_h,
  ...
  mop_mesh_set_material(floor_mesh,
                        &(MopMaterial){
  ...
  mop_viewport_set_camera(vp, (MopVec3){0, 5, 12}, (MopVec3){0, 1, 0},
  ...
  while (running) {
    ...
    while (SDL_PollEvent(&ev)) {
      ...
      case SDL_EVENT_MOUSE_BUTTON_DOWN:
        if (gs.mode == MODE_PLAY) {
          if (ev.button.button == SDL_BUTTON_LEFT) {
            ...
            game_shoot(vp, &gs, (float)win_w / 2.0f, (float)win_h / 2.0f);
          }
        } else {
      ...
      case SDL_EVENT_MOUSE_BUTTON_UP:
        if (gs.mode == MODE_EDITOR) {
          if (ev.button.button == SDL_BUTTON_LEFT) {
            ...
            mop_viewport_input(
                vp, &(MopInputEvent){.type = MOP_INPUT_SECONDARY_UP});
          }
        }
        break;
      ...
      case SDL_EVENT_MOUSE_MOTION:
        if (gs.mode == MODE_PLAY) {
          ...
          if (gs.cam.pitch > max_pitch)
            gs.cam.pitch = max_pitch;
          if (gs.cam.pitch < -max_pitch)
            gs.cam.pitch = -max_pitch;
        } else {
          /* Editor: forward motion for orbit/pan */
          mop_viewport_input(vp, &(MopInputEvent){MOP_INPUT_POINTER_MOVE,
    ...
    if (gs.mode == MODE_PLAY) {
      ...
      mop_viewport_set_camera(vp, fps_eye(&gs.cam), fps_target(&gs.cam),
                              (MopVec3){0, 1, 0}, 55.0f, 0.1f, 100.0f);
    }
