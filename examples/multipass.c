...
static void post_opaque_hook(MopViewport *vp, void *user_data) {
  ...
  if (ctx->verbose) {
    printf("  [frame %d] %-14s  tris=%u  visible=%u\n", ctx->frame,
           STAGE_NAMES[s], ctx->tri_count, ctx->visible);
  }
}
...
static void post_overlay_hook(MopViewport *vp, void *user_data) {
  ...
  if (px && w > 0 && h > 0) {
    ...
    for (int i = 0; i < total; i++) {
      ...
      sum += (uint64_t)px[idx] * 2 + (uint64_t)px[idx + 1] * 3 +
             (uint64_t)px[idx + 2];
    }
...
static void setup(MopViewport *vp, void *user) {
  ...
  for (int i = 0; i < 7; i++) {
    ...
    ctx->handles[i] = mop_viewport_add_hook(vp, (MopPipelineStage)i,
                                            hook_fns[i], &g_hook_data[i]);
    ctx->active[i] = true;
    ctx->counts[i] = 0;
  }
  ...
  if (key == SDLK_S) {
    ...
    for (int i = 0; i < 7; i++) {
      printf("  %d. %-14s  %s  calls=%d  handle=%u\n", i + 1, STAGE_NAMES[i],
             ctx->active[i] ? "ON " : "OFF", ctx->counts[i],
