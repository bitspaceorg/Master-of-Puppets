...
#define FACE_VERTS(nx,ny,nz, cr,cg,cb, x0,y0,z0, x1,y1,z1, x2,y2,z2, x3,y3,z3) \
    ...
    {{ .5f*(x1), .5f*(y1), .5f*(z1) }, { nx,ny,nz }, { cr,cg,cb,1 }, 0, 0},      \
    {{ .5f*(x2), .5f*(y2), .5f*(z2) }, { nx,ny,nz }, { cr,cg,cb,1 }, 0, 0},      \
    {{ .5f*(x3), .5f*(y3), .5f*(z3) }, { nx,ny,nz }, { cr,cg,cb,1 }, 0, 0},
    ...
    for (int row = 0; row <= PLANE_RES; row++) {
        for (int col = 0; col <= PLANE_RES; col++) {
            ...
            g_plane_verts[vi] = (FlexVertex){
                ...
                .cr = cr, .cg = 0.2f, .cb = cb, .ca = 1.0f,
                .u = u, .v = v,
                .h0 = heat, .h1 = 0.0f, .h2 = 0.0f, .h3 = 0.0f,
            };
            vi++;
        }
    }
...
int main(int argc, char *argv[]) {
    ...
    SDL_Window *window = SDL_CreateWindow(
        "MOP — Phase 1 Showcase", WINDOW_W, WINDOW_H,
    ...
    MopMesh *sphere = mop_viewport_add_mesh(vp, &(MopMeshDesc){
    ...
    MopVertexFormat flex_fmt = {
        ...
        .attribs = {
            { MOP_ATTRIB_POSITION,  MOP_FORMAT_FLOAT3, offsetof(FlexVertex, px) },
            { MOP_ATTRIB_NORMAL,    MOP_FORMAT_FLOAT3, offsetof(FlexVertex, nx) },
            { MOP_ATTRIB_COLOR,     MOP_FORMAT_FLOAT4, offsetof(FlexVertex, cr) },
            { MOP_ATTRIB_TEXCOORD0, MOP_FORMAT_FLOAT2, offsetof(FlexVertex, u)  },
            { MOP_ATTRIB_CUSTOM0,   MOP_FORMAT_FLOAT4, offsetof(FlexVertex, h0) },
            { 0 }, /* sentinel */
        },
    };

    MopMesh *plane = mop_viewport_add_mesh_ex(vp, &(MopMeshDescEx){
    ...
    int btn_key_light = ui_toolbar_button(&tb, "Key Light",  UI_BTN_TOGGLE, 0, true);
    ...
    int btn_wireframe = ui_toolbar_button(&tb, "Wireframe",  UI_BTN_TOGGLE, 0, false);
    ...
    int btn_selection = ui_toolbar_button(&tb, "Selection",  UI_BTN_TOGGLE, 0, true);
    ...
    int btn_auto_rotate = ui_toolbar_button(&tb, "Auto-Rotate", UI_BTN_TOGGLE, 0, true);
