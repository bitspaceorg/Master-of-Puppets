...
static const MopMaterial CUBE_MATERIALS[] = {
    { .base_color = { 0.9f, 0.2f, 0.2f, 1.0f }, .metallic = 0.0f, .roughness = 0.5f },
    { .base_color = { 0.2f, 0.8f, 0.3f, 1.0f }, .metallic = 0.0f, .roughness = 0.5f },
    { .base_color = { 0.2f, 0.3f, 0.9f, 1.0f }, .metallic = 0.0f, .roughness = 0.5f },
    { .base_color = { 0.9f, 0.8f, 0.1f, 1.0f }, .metallic = 0.2f, .roughness = 0.4f },
    { .base_color = { 0.8f, 0.2f, 0.8f, 1.0f }, .metallic = 0.1f, .roughness = 0.6f },
};
...
static void pick_setup(MopViewport *vp, void *data)
{
    ...
    printf("Controls: Click=raycast  F=frustum  A=AABB  W=wireframe  Q/Esc=quit\n\n");
}
...
static void pick_on_click(MopViewport *vp, float x, float y, void *data)
{
    ...
    if (hit.hit) {
        ...
        printf("    triangle = %u  uv = (%.3f, %.3f)\n",
               hit.triangle_index, hit.u, hit.v);
    } else {
        printf("  Raycast: MISS\n");
    }
    ...
    case SDLK_A: {
        ...
        for (uint32_t i = 0; i < mc; i++) {
            ...
            printf("    center=(%.3f,%.3f,%.3f)  area=%.3f\n",
                   mop_aabb_center(world).x,
                   mop_aabb_center(world).y,
                   mop_aabb_center(world).z,
                   mop_aabb_surface_area(world));
        }
        printf("\n");
        return true;
    }
