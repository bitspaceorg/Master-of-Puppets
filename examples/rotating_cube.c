...
int main(int argc, char *argv[]) {
  ...
  SDL_Window *window = SDL_CreateWindow("Master of Puppets", WINDOW_W, WINDOW_H,
  ...
  MopViewport *vp = mop_viewport_create(&(MopViewportDesc){
      .width = WINDOW_W, .height = WINDOW_H, .backend = MOP_BACKEND_CPU});
  if (!vp) {
  ...
  scene_add(scene, vp,
            &(MopMeshDesc){.vertices = CUBE_VERTS,
  ...
  printf(
      "  W: wireframe  |  Space: pause  |  R: reset  |  Esc: deselect/quit\n");
  ...
  while (running) {
    ...
    while (SDL_PollEvent(&ev)) {
      ...
      case SDL_EVENT_KEY_DOWN: {
        char key_buf[32];
        const char *key_name =
            sdl_key_to_config_name(ev.key.key, key_buf, sizeof(key_buf));
        const char *action =
            key_name ? mop_config_get_action(cfg, key_name) : NULL;
        ...
        if (action) {
          ...
          if (input >= 0) {
            ...
            if (input == MOP_INPUT_DESELECT && !mop_viewport_get_selected(vp)) {
              running = false;
            } else {
              mop_viewport_input(vp, &(MopInputEvent){.type = input});
            }
          }
          /* App-specific actions */
            ...
            SDL_ShowOpenFileDialog(obj_dialog_callback, &pending_obj_path,
                                   window, obj_filter, 1, NULL, false);
          }
        } else {
    ...
    /* ---- Continuous camera movement (arrow keys) ---- */
    {
      ...
      if (keys[SDL_SCANCODE_LEFT])
        rgt -= speed;
      if (fwd != 0 || rgt != 0)
        mop_viewport_input(vp, &(MopInputEvent){.type = MOP_INPUT_CAMERA_MOVE,
                                                .dx = rgt,
                                                .dy = fwd});
    }
    ...
    if (pending_obj_path) {
      ...
      if (mop_obj_load(pending_obj_path, &obj) &&
        ...
        SceneObject *o =
            scene_add(scene, vp,
                      &(MopMeshDesc){.vertices = obj.vertices,
        ...
        if (o)
          printf("Imported OBJ #%u (%u verts, %u tris) from %s\n", sid,
                 obj.vertex_count, obj.index_count / 3, pending_obj_path);
        mop_obj_free(&obj);
      } else {
