...
typedef enum { UI_BTN_TOGGLE, UI_BTN_MOMENTARY, UI_BTN_RADIO } UiBtnType;
...
typedef struct {
  ...
  enum {
    UI_ITEM_SECTION,
    UI_ITEM_BUTTON
  } items[UI_MAX_BUTTONS + UI_MAX_SECTIONS];
  int item_idx[UI_MAX_BUTTONS +
               UI_MAX_SECTIONS]; /* index into buttons[] or sections[] */
  int item_count;
...
static inline void ui_toolbar_section(UiToolbar *tb, const char *label) {
  if (tb->section_count >= UI_MAX_SECTIONS)
    return;
...
static inline int ui_toolbar_button(UiToolbar *tb, const char *label,
                                    ...
                                    bool initial_on) {
  if (tb->button_count >= UI_MAX_BUTTONS)
    return -1;
...
static inline bool ui_toolbar_is_on(const UiToolbar *tb, int btn_idx) {
  if (btn_idx < 0 || btn_idx >= tb->button_count)
    return false;
...
/* Toggle a button (respects type: toggle flips, radio selects, momentary
 * fires). */
...
static inline void ui_toolbar_render(const UiToolbar *tb, SDL_Renderer *r,
  ...
  SDL_RenderLine(r, (float)UI_SIDEBAR_WIDTH - 1, 0, (float)UI_SIDEBAR_WIDTH - 1,
                 (float)win_h);
  ...
  for (int i = 0; i < tb->item_count; i++) {
    if (tb->items[i] == UI_ITEM_SECTION) {
      ...
      SDL_RenderDebugText(r, (float)UI_BTN_PAD_LEFT / UI_TEXT_SCALE,
                          (s->y + 4.0f) / UI_TEXT_SCALE, s->label);
      ...
      SDL_FRect br = {(float)UI_BTN_PAD_LEFT, btn->y,
                      (float)(UI_SIDEBAR_WIDTH - 2 * UI_BTN_PAD_LEFT),
                      (float)UI_BTN_HEIGHT};

      /* Button background */
      if (btn->on &&
          (btn->type == UI_BTN_TOGGLE || btn->type == UI_BTN_RADIO)) {
      ...
      SDL_SetRenderScale(r, UI_TEXT_SCALE, UI_TEXT_SCALE);
      SDL_RenderDebugText(r, ((float)UI_BTN_PAD_LEFT + 8.0f) / UI_TEXT_SCALE,
                          (btn->y + 8.0f) / UI_TEXT_SCALE, btn->label);
      SDL_SetRenderScale(r, 1.0f, 1.0f);
    }
  }
}
