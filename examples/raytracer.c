...
static void btn_draw(SDL_Renderer *renderer, int win_w, bool show_rt,
                     bool hover) {
...
static MopColor shade_hit(MopTriangle *tri, float u, float v, MopVec3 hit_pos,
                          MopRay ray, const MopLight *lights,
  ...
  MopVec3 normal = mop_vec3_normalize(
      mop_vec3_add(mop_vec3_add(mop_vec3_scale(tri->n[0], w0),
...
static void raytrace_scene(MopViewport *vp, RaytracerCtx *ctx) {
  ...
  printf("[raytracer] Tracing %dx%d  (%u triangles, %u lights)\n", fb_w, fb_h,
         ctx->tri_count, snap.light_count);
  printf("[raytracer] Camera: eye=(%.2f,%.2f,%.2f) fov=%.1f deg\n", cam.eye.x,
         cam.eye.y, cam.eye.z, cam.fov_radians * 180.0f / 3.14159265f);
  ...
  for (int y = 0; y < fb_h; y++) {
    for (int x = 0; x < fb_w; x++) {
      MopRay ray =
          mop_viewport_pixel_to_ray(vp, (float)x + 0.5f, (float)y + 0.5f);
      ...
      for (uint32_t i = 0; i < ctx->tri_count; i++) {
        float t, u, v;
        bool hit = mop_ray_intersect_triangle(ray, ctx->tris[i].p[0],
                                              ctx->tris[i].p[1],
                                              ctx->tris[i].p[2], &t, &u, &v);
      ...
      if (best_id >= 0) {
        hits++;
        MopVec3 hit_pos =
            mop_vec3_add(ray.origin, mop_vec3_scale(ray.direction, best_t));

        MopColor c = shade_hit(&ctx->tris[best_id], best_u, best_v, hit_pos,
  ...
  printf("  Hits:               %u (%.1f%%)\n", hits,
         100.0f * (float)hits / (float)total_rays);
...
int main(int argc, char *argv[]) {
  ...
  SDL_Window *window = SDL_CreateWindow("MOP — CPU Raytracer", win_w, win_h,
  ...
  /* ---- Camera ---- */
  mop_viewport_set_camera(vp, (MopVec3){3.0f, 2.0f, 4.0f},
                          (MopVec3){0.0f, 0.4f, 0.0f},
                          (MopVec3){0.0f, 1.0f, 0.0f}, 60.0f, 0.1f, 100.0f);

  mop_viewport_set_clear_color(vp, (MopColor){0.05f, 0.05f, 0.15f, 1.0f});
  ...
  printf(
      "[raytracer] Scene: 3 cubes + floor, 2 lights (directional + point)\n");
  printf("[raytracer] Controls: R=raytrace  M=MOP view  W=wireframe  "
         "Q/Esc=quit\n\n");
  ...
  while (running) {
    ...
    while (SDL_PollEvent(&ev)) {
      ...
      case SDL_EVENT_MOUSE_MOTION:
        ...
        ctx.btn_hover = btn_contains(btn_rect(win_w), ev.motion.x, ev.motion.y);
        mop_viewport_input(vp,
    ...
    if (ctx.show_rt && ctx.rt_valid && ctx.rt_fb && ctx.rt_w == win_w &&
        ctx.rt_h == win_h) {
