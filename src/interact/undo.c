...
void mop_viewport_push_undo(MopViewport *viewport, MopMesh *mesh) {
  ...
  viewport->undo_entries[slot] = (MopUndoEntry){.mesh_index = idx,
                                                .pos = mesh->position,
                                                .rot = mesh->rotation,
...
void mop_viewport_undo(MopViewport *viewport) {
  if (!viewport || viewport->undo_count == 0)
    return;
  ...
  if (entry->mesh_index >= viewport->mesh_count)
    return;
...
void mop_viewport_redo(MopViewport *viewport) {
  if (!viewport || viewport->redo_count == 0)
    return;
