...
bool mop_binary_load(const char *path, MopBinaryMesh *out) {
  ...
  if (hdr->version != MOP_BINARY_VERSION) {
    munmap(mapping, file_size);
    MOP_ERROR("mop_binary_load: unsupported version %u in '%s'", hdr->version,
              path);
    return false;
  }
  ...
  out->bbox_min =
      (MopVec3){hdr->bbox_min[0], hdr->bbox_min[1], hdr->bbox_min[2]};
  out->bbox_max =
      (MopVec3){hdr->bbox_max[0], hdr->bbox_max[1], hdr->bbox_max[2]};
