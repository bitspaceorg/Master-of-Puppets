...
static MopVec3 compute_normal(MopVec3 a, MopVec3 b, MopVec3 c) {
  ...
  MopVec3 n = {e1.y * e2.z - e1.z * e2.y, e1.z * e2.x - e1.x * e2.z,
               e1.x * e2.y - e1.y * e2.x};
...
bool mop_obj_load(const char *path, MopObjMesh *out) {
  ...
  while (fgets(line, sizeof(line), fp)) {
    ...
    if (line[0] == 'v' && line[1] == ' ') {
      ...
      for (int i = 0; i < face_count; i++) {
        ...
        verts[vert_count++] =
            (MopVertex){pos, norm, default_color, tex_u, tex_v};
      }

      /* If no normals were provided, compute face normal */
      if (face[0].ni < 0) {
        MopVec3 fn =
            compute_normal(verts[base].position, verts[base + 1].position,
  ...
  if (tangents) {
    for (uint32_t i = 0; i + 2 < indices.n; i += 3) {
      ...
      MopVec3 tan = {(edge1.x * dv2 - edge2.x * dv1) * inv_det,
                     (edge1.y * dv2 - edge2.y * dv1) * inv_det,
                     (edge1.z * dv2 - edge2.z * dv1) * inv_det};

      /* Accumulate tangent to all 3 vertices */
      tangents[idx0].x += tan.x;
      tangents[idx0].y += tan.y;
      tangents[idx0].z += tan.z;
      tangents[idx1].x += tan.x;
      tangents[idx1].y += tan.y;
      tangents[idx1].z += tan.z;
      tangents[idx2].x += tan.x;
      tangents[idx2].y += tan.y;
      tangents[idx2].z += tan.z;
    }

    /* Normalize accumulated tangents */
    for (uint32_t i = 0; i < vert_count; i++) {
      float len =
          sqrtf(tangents[i].x * tangents[i].x + tangents[i].y * tangents[i].y +
