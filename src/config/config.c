...
MopConfig *mop_config_load(const char *path) {
  ...
  if (lua_istable(L, -1)) {
    double v;
    if (lua_get_number(L, "width", &v)) {
      cfg->has_width = true;
      cfg->width = (int)v;
    }
    if (lua_get_number(L, "height", &v)) {
      cfg->has_height = true;
      cfg->height = (int)v;
    }
  ...
  if (lua_istable(L, -1)) {
    double v;
    if (lua_get_number(L, "distance", &v)) {
      cfg->has_cam_distance = true;
      cfg->cam_distance = (float)v;
    }
    if (lua_get_number(L, "yaw", &v)) {
      cfg->has_cam_yaw = true;
      cfg->cam_yaw = (float)v;
    }
    if (lua_get_number(L, "pitch", &v)) {
      cfg->has_cam_pitch = true;
      cfg->cam_pitch = (float)v;
    }
    if (lua_get_number(L, "fov", &v)) {
      cfg->has_cam_fov = true;
      cfg->cam_fov = (float)v;
    }
...
void mop_config_free(MopConfig *cfg) { free(cfg); }
...
void mop_config_apply(const MopConfig *cfg, MopViewport *vp) {
  ...
  if (cfg->has_cam_distance)
    vp->camera.distance = cfg->cam_distance;
  if (cfg->has_cam_yaw)
    vp->camera.yaw = cfg->cam_yaw;
  if (cfg->has_cam_pitch)
    vp->camera.pitch = cfg->cam_pitch;
  if (cfg->has_cam_target)
    vp->camera.target = cfg->cam_target;
  if (cfg->has_cam_fov)
    vp->camera.fov_degrees = cfg->cam_fov;
}
