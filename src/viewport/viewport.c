...
static MopMesh *create_grid(MopViewport *vp) {
    ...
    if (!v || !ix) { free(v); free(ix); return NULL; }
    ...
    for (int i = 0; i < lines_per_axis; i++) {
        ...
        MopColor c;
        float hw;
        if (ix_val == 0)            { c = c_z; hw = GRID_HW_AXIS; }
        else if (ix_val % 5 == 0)   { c = c_major; hw = GRID_HW_MAJOR; }
        else                         { c = c_minor; hw = GRID_HW_MINOR; }
        int b = vi;
        v[vi++] = (MopVertex){{ x - hw, 0.0f, -fext }, n, c, 0, 0};
        v[vi++] = (MopVertex){{ x + hw, 0.0f, -fext }, n, c, 0, 0};
        v[vi++] = (MopVertex){{ x + hw, 0.0f,  fext }, n, c, 0, 0};
        v[vi++] = (MopVertex){{ x - hw, 0.0f,  fext }, n, c, 0, 0};
        ix[ii++]=(uint32_t)b;   ix[ii++]=(uint32_t)(b+1); ix[ii++]=(uint32_t)(b+2);
        ix[ii++]=(uint32_t)(b+2); ix[ii++]=(uint32_t)(b+3); ix[ii++]=(uint32_t)b;
    }
    ...
    for (int i = 0; i < lines_per_axis; i++) {
        ...
        MopColor c;
        float hw;
        if (iz_val == 0)            { c = c_x; hw = GRID_HW_AXIS; }
        else if (iz_val % 5 == 0)   { c = c_major; hw = GRID_HW_MAJOR; }
        else                         { c = c_minor; hw = GRID_HW_MINOR; }
        int b = vi;
    ...
    MopMesh *grid = mop_viewport_add_mesh(vp, &(MopMeshDesc){
        .vertices     = v,
...
static void create_gradient_bg(MopViewport *vp) {
    ...
    MopRhiBufferDesc vb_desc = {
        .data = verts, .size = sizeof(verts)
    };
    MopRhiBufferDesc ib_desc = {
        .data = indices, .size = sizeof(indices)
    };
...
static void create_one_axis(MopViewport *vp, int idx,
                             MopVec3 dir, MopVec3 perp1, MopVec3 perp2,
                             MopColor color) {
    ...
    MopVertex verts[13];
    uint32_t  indices[30];  /* 2 quads × 6 + 4 tri faces × 3 + 2 base tris × 3 = 12 + 12 + 6 = 30 */
    int vi = 0, ii = 0;
    ...
    verts[vi++] = (MopVertex){{
        s0.x - perp1.x * shaft_hw, s0.y - perp1.y * shaft_hw, s0.z - perp1.z * shaft_hw
    }, n, c, 0, 0};
    verts[vi++] = (MopVertex){{
        s0.x + perp1.x * shaft_hw, s0.y + perp1.y * shaft_hw, s0.z + perp1.z * shaft_hw
    }, n, c, 0, 0};
    verts[vi++] = (MopVertex){{
        s1.x + perp1.x * shaft_hw, s1.y + perp1.y * shaft_hw, s1.z + perp1.z * shaft_hw
    }, n, c, 0, 0};
    verts[vi++] = (MopVertex){{
        s1.x - perp1.x * shaft_hw, s1.y - perp1.y * shaft_hw, s1.z - perp1.z * shaft_hw
    }, n, c, 0, 0};
    ...
    verts[vi++] = (MopVertex){{
        s0.x - perp2.x * shaft_hw, s0.y - perp2.y * shaft_hw, s0.z - perp2.z * shaft_hw
    }, n, c, 0, 0};
    verts[vi++] = (MopVertex){{
        s0.x + perp2.x * shaft_hw, s0.y + perp2.y * shaft_hw, s0.z + perp2.z * shaft_hw
    }, n, c, 0, 0};
    verts[vi++] = (MopVertex){{
        s1.x + perp2.x * shaft_hw, s1.y + perp2.y * shaft_hw, s1.z + perp2.z * shaft_hw
    }, n, c, 0, 0};
    verts[vi++] = (MopVertex){{
        s1.x - perp2.x * shaft_hw, s1.y - perp2.y * shaft_hw, s1.z - perp2.z * shaft_hw
    }, n, c, 0, 0};
    indices[ii++]=4; indices[ii++]=5; indices[ii++]=6;
    indices[ii++]=6; indices[ii++]=7; indices[ii++]=4;
    ...
    verts[vi++] = (MopVertex){{
        base_c.x - perp1.x * tip_hw, base_c.y - perp1.y * tip_hw, base_c.z - perp1.z * tip_hw
    }, n, c, 0, 0};  /* 9 */
    verts[vi++] = (MopVertex){{
        base_c.x + perp2.x * tip_hw, base_c.y + perp2.y * tip_hw, base_c.z + perp2.z * tip_hw
    }, n, c, 0, 0};  /* 10 */
    verts[vi++] = (MopVertex){{
        base_c.x + perp1.x * tip_hw, base_c.y + perp1.y * tip_hw, base_c.z + perp1.z * tip_hw
    }, n, c, 0, 0};  /* 11 */
    ...
    indices[ii++]=(uint32_t)a;  indices[ii++]=(uint32_t)b0; indices[ii++]=(uint32_t)b1;
    indices[ii++]=(uint32_t)a;  indices[ii++]=(uint32_t)b1; indices[ii++]=(uint32_t)b2;
    indices[ii++]=(uint32_t)a;  indices[ii++]=(uint32_t)b2; indices[ii++]=(uint32_t)b3;
    indices[ii++]=(uint32_t)a;  indices[ii++]=(uint32_t)b3; indices[ii++]=(uint32_t)b0;
    /* Base (2 tris) */
    indices[ii++]=(uint32_t)b0; indices[ii++]=(uint32_t)b2; indices[ii++]=(uint32_t)b1;
    indices[ii++]=(uint32_t)b0; indices[ii++]=(uint32_t)b3; indices[ii++]=(uint32_t)b2;

    MopRhiBufferDesc vb_desc = { .data = verts, .size = (uint32_t)vi * sizeof(MopVertex) };
    MopRhiBufferDesc ib_desc = { .data = indices, .size = (uint32_t)ii * sizeof(uint32_t) };
...
static void create_axis_indicator(MopViewport *vp) {
    create_one_axis(vp, 0,
        (MopVec3){1,0,0}, (MopVec3){0,1,0}, (MopVec3){0,0,1},
        (MopColor){0.85f, 0.20f, 0.20f, 1.0f});  /* X = red */
    create_one_axis(vp, 1,
        (MopVec3){0,1,0}, (MopVec3){1,0,0}, (MopVec3){0,0,1},
        (MopColor){0.30f, 0.75f, 0.30f, 1.0f});  /* Y = green */
    create_one_axis(vp, 2,
        (MopVec3){0,0,1}, (MopVec3){1,0,0}, (MopVec3){0,1,0},
...
MopViewport *mop_viewport_create(const MopViewportDesc *desc) {
    ...
    vp->instanced_meshes = calloc(MOP_INITIAL_INSTANCED_CAPACITY,
...
void mop_viewport_set_ambient(MopViewport *viewport, float ambient) {
    if (!viewport) return;
    viewport->ambient = (ambient < 0.0f) ? 0.0f : (ambient > 1.0f) ? 1.0f : ambient;
...
MopVec3 mop_viewport_get_camera_eye(const MopViewport *viewport) {
    if (!viewport) return (MopVec3){0, 0, 0};
...
MopMesh *mop_viewport_add_mesh_ex(MopViewport *viewport,
    ...
    const MopVertexAttrib *color_attr = mop_vertex_format_find(
        desc->vertex_format, MOP_ATTRIB_COLOR);
    if (color_attr && color_attr->format == MOP_FORMAT_FLOAT4) {
        float r_sum = 0, g_sum = 0, b_sum = 0;
        const uint8_t *raw = (const uint8_t *)desc->vertex_data;
        for (uint32_t i = 0; i < desc->vertex_count; i++) {
            const float *c = (const float *)(raw + (size_t)i * desc->vertex_format->stride + color_attr->offset);
...
void mop_mesh_update_geometry(MopMesh *mesh, MopViewport *viewport,
    ...
    if (vertex_count <= mesh->vertex_capacity) {
        ...
        viewport->rhi->buffer_update(viewport->device, mesh->vertex_buffer,
                                     vertices, 0,
                                     vertex_count * sizeof(MopVertex));
    } else {
        /* Grow: destroy old, create new with 2x capacity */
        viewport->rhi->buffer_destroy(viewport->device, mesh->vertex_buffer);
        uint32_t new_cap = mesh->vertex_capacity;
        while (new_cap < vertex_count) new_cap = new_cap ? new_cap * 2 : 64;

        void *tmp = calloc(new_cap, sizeof(MopVertex));
        if (!tmp) return;
        memcpy(tmp, vertices, vertex_count * sizeof(MopVertex));
        MopRhiBufferDesc vb_desc = {
            .data = tmp, .size = new_cap * sizeof(MopVertex)
        };
        mesh->vertex_buffer = viewport->rhi->buffer_create(
            viewport->device, &vb_desc);
        free(tmp);
        if (!mesh->vertex_buffer) { mesh->active = false; return; }
    ...
    if (index_count <= mesh->index_capacity) {
        viewport->rhi->buffer_update(viewport->device, mesh->index_buffer,
                                     indices, 0,
                                     index_count * sizeof(uint32_t));
    } else {
        viewport->rhi->buffer_destroy(viewport->device, mesh->index_buffer);
        uint32_t new_cap = mesh->index_capacity;
        while (new_cap < index_count) new_cap = new_cap ? new_cap * 2 : 64;

        void *tmp = calloc(new_cap, sizeof(uint32_t));
        if (!tmp) return;
        memcpy(tmp, indices, index_count * sizeof(uint32_t));
        MopRhiBufferDesc ib_desc = {
            .data = tmp, .size = new_cap * sizeof(uint32_t)
        };
        mesh->index_buffer = viewport->rhi->buffer_create(
            viewport->device, &ib_desc);
        free(tmp);
        if (!mesh->index_buffer) { mesh->active = false; return; }
...
MopTexture *mop_viewport_create_texture(MopViewport *viewport, int width,
    ...
    if (!viewport || width <= 0 || height <= 0 || !rgba_data) return NULL;

    MopRhiTexture *rhi_tex = viewport->rhi->texture_create(
        viewport->device, width, height, rgba_data);
    if (!rhi_tex) return NULL;
...
#define EMIT_DRAW(vp, mesh_ptr) do { \
    ...
    MopRhiDrawCall call_ = { \
        ...
        .metallic      = m_->has_material \
                         ? m_->material.metallic : 0.0f, \
        .roughness     = m_->has_material \
                         ? m_->material.roughness : 0.5f, \
        .emissive      = m_->has_material \
                         ? m_->material.emissive \
                         : (MopVec3){0, 0, 0}, \
        .lights        = (vp)->lights, \
...
static void pass_scene_opaque(MopViewport *vp) {
    for (uint32_t i = 0; i < vp->mesh_count; i++) {
        ...
        if (mesh->blend_mode != MOP_BLEND_OPAQUE) continue;
...
static void pass_overlays(MopViewport *vp) {
    ...
    if (vp->display.show_normals &&
        vp->overlay_enabled[MOP_OVERLAY_NORMALS]) {
        mop_overlay_builtin_normals(vp, NULL);
    }
    if (vp->display.show_bounds &&
        vp->overlay_enabled[MOP_OVERLAY_BOUNDS]) {
...
static void pass_hud(MopViewport *vp) {
    ...
    for (int ax = 0; ax < 3; ax++) {
        ...
        MopColor ax_color;
        if (ax == 0)      ax_color = (MopColor){0.85f, 0.20f, 0.20f, 1.0f};
        else if (ax == 1) ax_color = (MopColor){0.30f, 0.75f, 0.30f, 1.0f};
        else              ax_color = (MopColor){0.25f, 0.40f, 0.90f, 1.0f};
...
static void pass_instanced(MopViewport *vp) {
    for (uint32_t i = 0; i < vp->instanced_count; i++) {
        ...
        if (!im->active || im->instance_count == 0) continue;
        ...
        MopRhiDrawCall inst_call = {
            ...
            .texture       = im->texture
                             ? im->texture->rhi_texture : NULL,
            .blend_mode    = im->blend_mode,
            .metallic      = im->has_material
                             ? im->material.metallic : 0.0f,
            .roughness     = im->has_material
                             ? im->material.roughness : 0.5f,
            .emissive      = im->has_material
                             ? im->material.emissive
                             : (MopVec3){0, 0, 0},
            .lights        = vp->lights,
            .light_count   = vp->light_count,
            .vertex_format = NULL,
        ...
        vp->rhi->draw_instanced(vp->device, vp->framebuffer,
                                 &inst_call, im->transforms,
                                 im->instance_count);
    }
}
...
void mop_viewport_render(MopViewport *viewport) {
    ...
    for (uint32_t i = 0; i < viewport->mesh_count; i++) {
        ...
        if (!mesh->active) continue;
        if (mesh->use_trs) {
            mesh->transform = mop_mat4_compose_trs(
                mesh->position, mesh->rotation, mesh->scale_val);
        }
    }
    ...
    for (int pass = 0; pass < 16; pass++) {
        ...
        for (uint32_t i = 0; i < viewport->mesh_count; i++) {
            struct MopMesh *mesh = &viewport->meshes[i];
            if (!mesh->active || mesh->parent_index < 0) continue;
...
void mop_mesh_set_parent(MopMesh *mesh, MopMesh *parent, MopViewport *viewport) {
    if (!mesh || !parent || !viewport) return;
    ...
    if (parent_idx < 0) return;  /* parent not found in this viewport */
...
MopInstancedMesh *mop_viewport_add_instanced_mesh(MopViewport *viewport,
    ...
    if (slot == viewport->instanced_count) {
        ...
        if (viewport->instanced_count >= viewport->instanced_capacity) {
            ...
            struct MopInstancedMesh *new_arr = realloc(
                viewport->instanced_meshes,
