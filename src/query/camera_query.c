...
MopRay mop_viewport_pixel_to_ray(const MopViewport *vp, float x, float y) {
  ...
  /* Perspective divide */
  if (fabsf(near_world.w) < 1e-8f || fabsf(far_world.w) < 1e-8f)
    return ray;

  float inv_nw = 1.0f / near_world.w;
  float inv_fw = 1.0f / far_world.w;

  MopVec3 near_pt = {near_world.x * inv_nw, near_world.y * inv_nw,
                     near_world.z * inv_nw};
  MopVec3 far_pt = {far_world.x * inv_fw, far_world.y * inv_fw,
                    far_world.z * inv_fw};
