...
bool mop_snapshot_next_mesh(MopSceneSnapshot *snap, MopMeshView *out) {
    if (!snap || !out || !snap->_vp) return false;
    ...
    while (snap->_mesh_idx < vp->mesh_count) {
        ...
        if (m->vertex_buffer && !m->vertex_format) {
            out->vertices = (const MopVertex *)vp->rhi->buffer_read(
                                m->vertex_buffer);
        } else {
            out->vertices = NULL;
        }
        if (m->index_buffer) {
            out->indices = (const uint32_t *)vp->rhi->buffer_read(
                               m->index_buffer);
        } else {
            out->indices = NULL;
        }
...
uint32_t mop_snapshot_triangle_count(const MopSceneSnapshot *snap) {
    ...
    for (uint32_t i = 0; i < vp->mesh_count; i++) {
        ...
        if (is_scene_mesh(m)) total += m->index_count / 3;
    }
    return total;
}
...
MopTriangleIter mop_triangle_iter_begin(const MopViewport *vp) {
    ...
    if (iter._has_mesh) {
        iter._normal_matrix = compute_normal_matrix(
            iter._current_mesh.world_transform);
    }
    return iter;
}
    ...
    while (iter->_has_mesh) {
        ...
        if (iter->_tri_idx < tri_count && mv->vertices && mv->indices) {
            ...
            for (int k = 0; k < 3; k++) {
                ...
                MopVec4 wp = mop_mat4_mul_vec4(w,
                    (MopVec4){ v->position.x, v->position.y,
                               v->position.z, 1.0f });
                out->p[k] = (MopVec3){ wp.x, wp.y, wp.z };

                /* Transform normal (no translation, normalize) */
                MopVec4 wn = mop_mat4_mul_vec4(nm,
                    (MopVec4){ v->normal.x, v->normal.y,
                               v->normal.z, 0.0f });
                out->n[k] = mop_vec3_normalize(
                    (MopVec3){ wn.x, wn.y, wn.z });
        ...
        iter->_tri_idx = 0;
        iter->_has_mesh = mop_snapshot_next_mesh(&iter->_snap,
                                                  &iter->_current_mesh);
        if (iter->_has_mesh) {
            iter->_normal_matrix = compute_normal_matrix(
                iter->_current_mesh.world_transform);
