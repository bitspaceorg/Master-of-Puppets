...
MopAABB mop_mesh_get_aabb_local(const MopMesh *mesh, const MopViewport *vp) {
  ...
  if (mesh->vertex_format) {
    ...
    const MopVertexAttrib *pos_attr =
        mop_vertex_format_find(mesh->vertex_format, MOP_ATTRIB_POSITION);
    if (!pos_attr)
      return box;
    ...
    for (uint32_t i = 1; i < count; i++) {
      const float *p =
          (const float *)(bytes + (size_t)i * stride + pos_attr->offset);
      if (p[0] < box.min.x)
        box.min.x = p[0];
      if (p[1] < box.min.y)
        box.min.y = p[1];
...
MopAABB mop_mesh_get_aabb_world(const MopMesh *mesh, const MopViewport *vp) {
  ...
  MopVec4 first = mop_mat4_mul_vec4(
      w, (MopVec4){corners[0].x, corners[0].y, corners[0].z, 1.0f});
  MopAABB result = {.min = {first.x, first.y, first.z},
                    .max = {first.x, first.y, first.z}};

  for (int i = 1; i < 8; i++) {
    MopVec4 tp = mop_mat4_mul_vec4(
        w, (MopVec4){corners[i].x, corners[i].y, corners[i].z, 1.0f});
    if (tp.x < result.min.x)
      result.min.x = tp.x;
    if (tp.y < result.min.y)
      result.min.y = tp.y;
    if (tp.z < result.min.z)
      result.min.z = tp.z;
    if (tp.x > result.max.x)
      result.max.x = tp.x;
    if (tp.y > result.max.y)
      result.max.y = tp.y;
    if (tp.z > result.max.z)
      result.max.z = tp.z;
  }
...
MopVec3 mop_aabb_extents(MopAABB box) {
  return (MopVec3){(box.max.x - box.min.x) * 0.5f,
...
#undef R
  ...
  for (int i = 0; i < 6; i++) {
    float len =
        sqrtf(f.planes[i].x * f.planes[i].x + f.planes[i].y * f.planes[i].y +
...
int mop_frustum_test_aabb(const MopFrustum *frustum, MopAABB box) {
  ...
  for (int i = 0; i < 6; i++) {
    ...
    if (nx * pv.x + ny * pv.y + nz * pv.z + d < 0)
      return -1;

    /* If negative vertex is outside, AABB intersects this plane */
    if (nx * nv.x + ny * nv.y + nz * nv.z + d < 0)
      all_inside = false;
  }
...
MopRayHit mop_viewport_raycast_ray(const MopViewport *vp, MopRay ray) {
  ...
  for (uint32_t mi = 0; mi < vp->mesh_count; mi++) {
    ...
    if (!mesh->vertex_buffer || !mesh->index_buffer)
      continue;
    if (mesh->vertex_format)
      continue; /* skip flex-format for now */
    ...
    for (uint32_t ti = 0; ti < tri_count; ti++) {
      ...
      MopVec4 wp0 = mop_mat4_mul_vec4(w, (MopVec4){verts[i0].position.x,
                                                   verts[i0].position.y,
                                                   verts[i0].position.z, 1.0f});
      MopVec4 wp1 = mop_mat4_mul_vec4(w, (MopVec4){verts[i1].position.x,
                                                   verts[i1].position.y,
                                                   verts[i1].position.z, 1.0f});
      MopVec4 wp2 = mop_mat4_mul_vec4(w, (MopVec4){verts[i2].position.x,
                                                   verts[i2].position.y,
      ...
      if (mop_ray_intersect_triangle(ray, p0, p1, p2, &t, &u, &v)) {
        if (t < closest_t) {
          ...
          result.distance = t;
          result.position =
              mop_vec3_add(ray.origin, mop_vec3_scale(ray.direction, t));
          result.u = u;
          result.v = v;
          ...
          result.normal = mop_vec3_normalize(mop_vec3_cross(e1, e2));
        }
      }
    }
  }
