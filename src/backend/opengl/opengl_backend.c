...
static MopRhiDevice *gl_device_create(void) {
  ...
  /* Compile shaders */
  dev->solid_program =
      create_program(MOP_GL_VERTEX_SHADER, MOP_GL_FRAGMENT_SHADER);
  dev->wireframe_program =
      create_program(MOP_GL_VERTEX_SHADER, MOP_GL_WIREFRAME_FRAGMENT_SHADER);

  if (!dev->solid_program || !dev->wireframe_program) {
    if (dev->solid_program)
      glDeleteProgram(dev->solid_program);
    if (dev->wireframe_program)
      glDeleteProgram(dev->wireframe_program);
    free(dev);
    return NULL;
  }
  ...
  dev->u_has_texture =
      glGetUniformLocation(dev->solid_program, "u_has_texture");
  ...
  dev->uw_object_id =
      glGetUniformLocation(dev->wireframe_program, "u_object_id");
...
static void gl_device_destroy(MopRhiDevice *device) {
  if (!device)
    return;
  if (device->solid_program)
    glDeleteProgram(device->solid_program);
  if (device->wireframe_program)
    glDeleteProgram(device->wireframe_program);
  if (device->empty_vao)
    glDeleteVertexArrays(1, &device->empty_vao);
  free(device);
}
...
static void gl_fb_create_attachments(struct MopRhiFramebuffer *fb, int width,
                                     int height) {
  fb->width = width;
  fb->height = height;
...
static void gl_fb_destroy_attachments(struct MopRhiFramebuffer *fb) {
  ...
  if (fb->color_tex)
    glDeleteTextures(1, &fb->color_tex);
  if (fb->pick_tex)
    glDeleteTextures(1, &fb->pick_tex);
  if (fb->depth_rbo)
...
gl_framebuffer_create(MopRhiDevice *device, const MopRhiFramebufferDesc *desc) {
  (void)device;
...
static void gl_framebuffer_resize(MopRhiDevice *device, MopRhiFramebuffer *fb,
...
static void gl_draw(MopRhiDevice *device, MopRhiFramebuffer *fb,
  ...
  if (call->wireframe) {
    ...
    glUniform3f(device->u_light_dir, call->light_dir.x, call->light_dir.y,
                call->light_dir.z);
    ...
    glUniform1i(device->u_num_lights, (call->lights && nl > 0) ? (GLint)nl : 0);
    if (call->lights && nl > 0) {
      for (uint32_t li = 0; li < nl; li++) {
        const MopLight *light = &call->lights[li];
        glUniform4f(device->u_light_position[li], light->position.x,
                    light->position.y, light->position.z, (float)light->type);
        glUniform4f(device->u_light_direction[li], light->direction.x,
                    light->direction.y, light->direction.z, 0.0f);
        glUniform4f(device->u_light_color[li], light->color.r, light->color.g,
                    light->color.b, light->intensity);
        glUniform4f(device->u_light_params[li], light->range,
                    light->spot_inner_cos, light->spot_outer_cos,
  ...
  glDrawElements(GL_TRIANGLES, (GLsizei)call->index_count, GL_UNSIGNED_INT,
                 NULL);
...
static const uint8_t *gl_framebuffer_read_color(MopRhiDevice *device,
  ...
  for (int y = 0; y < h; y++) {
    ...
    memcpy(fb->readback_color + (size_t)y * (size_t)w * 4, row, (size_t)w * 4);
  }
  free(row);
