...
static void tile_bin_push(MopTileBin *bin, uint32_t tri_idx) {
    if (bin->count == bin->capacity) {
        ...
        uint32_t *new_arr = realloc(bin->tri_indices,
                                     new_cap * sizeof(uint32_t));
        if (!new_arr) return;  /* drop triangle on OOM — non-fatal */
...
static void process_tile(const MopSwTileWork *work, int tile_idx) {
    ...
    for (uint32_t i = 0; i < bin->count; i++) {
        ...
        if (tri->lights && tri->light_count > 0) {
            mop_sw_rasterize_triangle_full(tri->vertices,
                                            tri->object_id,
                                            tri->wireframe,
                                            tri->depth_test,
                                            tri->cull_back,
            ...
            mop_sw_rasterize_triangle(tri->vertices,
                                       tri->object_id,
                                       tri->wireframe,
                                       tri->depth_test,
                                       tri->cull_back,
...
static void *worker_func(void *arg) {
    ...
    for (;;) {
        ...
        for (;;) {
            int tile_idx = __atomic_fetch_add(&work->next_tile, 1,
                                               __ATOMIC_RELAXED);
            if (tile_idx >= work->total_tiles) break;
...
static void bin_triangles(MopTileGrid *grid,
                          const MopSwPreparedTri *triangles,
    ...
    for (uint32_t t = 0; t < triangle_count; t++) {
        ...
        for (int vi = 0; vi < 3; vi++) {
            ...
            if (fabsf(w) < 1e-7f) { valid = false; break; }
            float inv_w = 1.0f / w;
        ...
        if (tile_x >= grid->tiles_x) tile_x = grid->tiles_x - 1;
        if (tile_y >= grid->tiles_y) tile_y = grid->tiles_y - 1;
...
void mop_sw_rasterize_tiled(MopSwThreadPool *pool,
                             ...
                             uint32_t triangle_count,
                             MopSwFramebuffer *fb) {
    if (!pool || !triangles || triangle_count == 0 || !fb) return;
