...
static void water_subsys_update(MopSubsystem *self, MopViewport *vp, float dt,
                                float t);
...
static float water_height(float x, float z, float t, float speed,
...
void mop_water_update(MopWaterSurface *ws, float t) {
  ...
  for (int z = 0; z < res; z++) {
    for (int x = 0; x < res; x++) {
      ...
      ws->vertices[idx].position.y = water_height(px, pz, t, spd, amp, freq);
    }
  }
  ...
  for (int z = 0; z < res; z++) {
    for (int x = 0; x < res; x++) {
      ...
      MopVec3 n = {(hL - hR) / (2.0f * eps), 1.0f, (hD - hU) / (2.0f * eps)};
  ...
  if (ws->vertex_buffer && ws->viewport->rhi->buffer_update) {
    ws->viewport->rhi->buffer_update(ws->viewport->device, ws->vertex_buffer,
                                     ws->vertices, 0,
                                     ws->vertex_count * sizeof(MopVertex));
  } else if (ws->vertex_buffer) {
    /* Fallback: destroy and recreate */
    ws->viewport->rhi->buffer_destroy(ws->viewport->device, ws->vertex_buffer);
    MopRhiBufferDesc vb_desc = {.data = ws->vertices,
                                .size = ws->vertex_count * sizeof(MopVertex)};
    ws->vertex_buffer =
        ws->viewport->rhi->buffer_create(ws->viewport->device, &vb_desc);
...
MopWaterSurface *mop_viewport_add_water(MopViewport *viewport,
  ...
  if (resolution > MOP_MAX_WATER_RESOLUTION) {
    MOP_WARN("water resolution %d capped to %d", resolution,
             MOP_MAX_WATER_RESOLUTION);
  ...
  if (viewport->water_count >= viewport->water_capacity) {
    uint32_t new_cap =
        viewport->water_capacity == 0 ? 4 : viewport->water_capacity * 2;
    MopWaterSurface **new_arr =
        realloc(viewport->water_surfaces, new_cap * sizeof(MopWaterSurface *));
    if (!new_arr) {
...
void mop_viewport_remove_water(MopViewport *viewport, MopWaterSurface *water) {
  if (!viewport || !water)
    return;
