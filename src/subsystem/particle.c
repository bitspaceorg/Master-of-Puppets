...
static void particle_subsys_update(MopSubsystem *self, MopViewport *vp,
                                   float dt, float t);
...
MopParticleEmitter *
mop_viewport_add_emitter(MopViewport *viewport,
  ...
  if (desc->max_particles > MOP_MAX_PARTICLES) {
    MOP_ERROR("max_particles %u exceeds limit %u", desc->max_particles,
              MOP_MAX_PARTICLES);
    return NULL;
  }
...
void mop_emitter_update(MopParticleEmitter *emitter, float dt,
  ...
  if (emitter->active && d->emit_rate > 0.0f) {
    ...
    for (uint32_t s = 0; s < to_spawn; s++) {
      ...
      p->lifetime = 0.0f;
      p->max_lifetime =
          rand_range(&emitter->rng_state, d->lifetime_min, d->lifetime_max);
      p->position = d->position;
      p->velocity.x =
          rand_range(&emitter->rng_state, d->velocity_min.x, d->velocity_max.x);
      p->velocity.y =
          rand_range(&emitter->rng_state, d->velocity_min.y, d->velocity_max.y);
      p->velocity.z =
          rand_range(&emitter->rng_state, d->velocity_min.z, d->velocity_max.z);
      p->size = d->size_start;
  ...
  for (uint32_t i = 0; i < emitter->max_particles; i++) {
    ...
    float t = (p->max_lifetime > 0.0f) ? (p->lifetime / p->max_lifetime) : 1.0f;
    ...
    MopVec3 bl = {p->position.x - r.x - u.x, p->position.y - r.y - u.y,
                  p->position.z - r.z - u.z};
    MopVec3 br = {p->position.x + r.x - u.x, p->position.y + r.y - u.y,
                  p->position.z + r.z - u.z};
    MopVec3 tr = {p->position.x + r.x + u.x, p->position.y + r.y + u.y,
                  p->position.z + r.z + u.z};
    MopVec3 tl = {p->position.x - r.x + u.x, p->position.y - r.y + u.y,
                  p->position.z - r.z + u.z};
...
void mop_emitter_get_mesh_data(const MopParticleEmitter *emitter,
  ...
  if (vertices)
    *vertices = emitter->verts;
  if (vertex_count)
    *vertex_count = emitter->vert_count;
  if (indices)
    *indices = emitter->idxs;
  if (index_count)
    *index_count = emitter->idx_count;
}
...
static void particle_subsys_update(MopSubsystem *self, MopViewport *vp,
  ...
  MopVec3 cam_right = {vp->view_matrix.d[0], vp->view_matrix.d[4],
                       vp->view_matrix.d[8]};
  MopVec3 cam_up = {vp->view_matrix.d[1], vp->view_matrix.d[5],
                    vp->view_matrix.d[9]};
